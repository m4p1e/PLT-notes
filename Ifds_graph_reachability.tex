...
\begin{definition}
\rm 设$\func{f}{2^D}{2^D}$某个IFDS problem中一个distribute transfer function，它的一个\redt{关系表示}$R_f \subseteq (D \cup \{0\}) \times (D \cup \{0\})$(representation relation)为
$$
\begin{aligned}
R_f = &~~~ \{0,0\} \\
...
\begin{theorem}
\rm 给定distribute transfer function $f$ (set union)，设$f$的关系表示为$R_f$，则
$$
...
\begin{proof}
\rm 上述representation correctness取决于$f$满足
$$
f(\{x_1\} \vee \{x_2\} \vee \cdots \vee \{x_{|D|}\}) = f(\{x_1\}) \vee f(\{x_2\}) \vee \cdots f(\{x_{|D|}\})
$$
其中$\vee$应为set union. 反过来也可以定义set intersection. 
\end{proof}
