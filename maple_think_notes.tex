...
\begin{example}
\rm 这是来自\cite{BMGM15}的一个小例子, 我们尝试用bounded model checking来做一些reasoning, 也就是unfolding loop. 
...
int checkValves(int wait1, int wait2) { 
	...
	while(wait2 > 0) wait2--; 
		return count;
\end{minted}
\cite{BMGM15}提到了一个symbolic reachability analysis, 实际上就是给定一个postcondition沿着control flow往后推. 例如我们想进入L29所在的branch, 那么one-step induction如下:
...
\begin{minted}[mathescape=true, tabsize=4, frame=single, fontsize=\small]{c}
//Path_1: $\cdots$ -> L$19_c$ -> L$20_c$ -> L$22_c$ -> L$23_c$ -> L$25_c$ -> L$19_b$ -> L$20_b$ -> L$22_b$ -> L$23_b$ -> L$25_b$ -> L$19_a$ -> L28
// $P_{18} = count > 2 \wedge i \geq size \wedge 0 > 2 \wedge 0 > size \equiv false$
{L18: count = 0, i = 0; }
// $Q_{18} = count > 2 \wedge i \geq size$
// $P_{19} = count > 2 \wedge i \geq size$
