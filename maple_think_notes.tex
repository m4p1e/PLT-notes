...
一个constant type family就是type $B(x)$并不依赖某个element with type $A$, 此时可以直接写作$B:\mathcal{U}$, $\mathcal{U}$也是可以盖住function types的. 
\end{example}
...
\begin{example}
\rm 当family $B$是constant的时候，有$\Pi_{(x:A)} B(x) \equiv (A \to B)$, 即original function type. 这说明dependent function type是可以完美盖住之前的function type的. 
\end{example}
