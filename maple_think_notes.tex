...
\begin{example}
\rm \redt{Symbolic reachability analysis} 这是来自\cite{BMGM15}的一个小例子, 我们尝试用bounded model checking来做一些reasoning, 也就是unfolding loop. 
...
int checkValves(int wait1, int wait2) { 
	...
	while(wait2 > 0) wait2--; 
		return count;
\end{minted}
\cite{BMGM15}提到了一个symbolic reachability analysis, 它和我们常见的symbolic execution是不一样的, 它可以看做给定一个postcondition沿着control flow往后推. 例如我们想进入L29所在的branch, 那么one-step induction如下:
