...
\textsc{Inductive case}: 当$|\text{atoms}(P)-\text{atoms}(Q)| = n+1$时，我们取$\alpha \in |\text{atoms}(P)-\text{atoms}(Q)|$.  我们定义$P_{\alpha \to \top}$表示将$P$中所有$\alpha$替换成$\top$得到的formula, 类似地我们用$P_{\alpha \to \bot}$表示将$P$中所有$\alpha$替换成$\bot$得到的formula. 显然我们有$P \equiv P_{\alpha \to \top} \vee P_{\alpha \to \bot}$. 根据inductive hypothesis我们找到一个关于$P_{\alpha \to \top} \vee P_{\alpha \to \bot} \to Q$的interpolation $C$. 显然$C$也是$P \to Q$的一个interpolation.
\end{proof}
...
\[
\forall x. F(x) \wedge G(y) \to G(y). 
\]
\cite{CF2007}里面提到了non-logical symbols.
\end{annotation}
