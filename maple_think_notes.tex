...
\begin{annotation}
\rm 前面第一个例子里面用到了给结点加label的概念, 我们可以给它形式化一下, 设label map $f_l: V_2 \to L$. 那么上面的例子中$L$为logic formulas $\mathcal{L}(\Sigma)$, 于是$G_2$上每一个结点都对应一个formula, 没有显式标注的label的结点, 都默认label为true.  
\end{annotation}
...
\begin{annotation}
\rm 当label刻画的是所在结点的状态的时候，$u$ is covered by $w$就是说$w$可以reachable的结点，那么$u$也一定是可以reachable的. 因为label越弱描述状态空间就越大. 
\end{annotation}
