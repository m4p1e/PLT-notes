...
\begin{proof}
在我的lattice notes中的complete lattice一节中的Knaster–Tarski theorem的开头证明里面证明了$\bigvee \text{Ext}(f) = \bigvee \text{Fix}(f)$.
\end{proof}

\begin{proposition}
\rm Given a order-preserving map $\func{f}{\mathcal{L}}{\mathcal{L}}$ on a complete lattice $\mathcal{L}$, then 
$$
f^\beta(\perp) \leq \bigvee\Set{f^\alpha(\perp)}{\alpha \in \mathbb{N}} \leq \text{lfp}(f) \leq \text{glp}(f) \leq  \bigwedge\Set{f^\alpha(\top)}{\alpha \in \mathbb{N}} \leq f^\beta(\top) .
$$
\end{proposition}

\begin{proof}
\rm 因$\perp \leq f(\perp)$，那么$\perp \in \text{Ext}(f)$，由于$f$是monotone的，那么$f(\perp) \leq f^2(\perp)$，推广至一般结论, 有$\Set{f^\alpha(\perp)}{\alpha \in \mathbb{N}} \subseteq \text{Ext}(f)$. 同理$\top \geq f(\top)$，推广至一般结论有$\Set{f^\alpha(\top)}{\alpha \in \mathbb{N}} \subseteq \text{Red}(f)$. 最后配合Lemma \ref{fixpoint-bound}即可证明. 
\end{proof}
