...
给定$v \in V$，设$B' = \Set{b \in B}{v~\mathcal{S}~b }$，证明$v \in \bigwedge B'$. 根据新的correctness relation定义，对任意的$b \in B'$都有$v~\mathcal{R}~g(b)$，那么再由$A$上的correctness relations有$v ~\mathcal{R}~(\bigwedge g(b))$，又因为$g$是一个meet semilattice homomorphism，那么在这里有$v~\mathcal{R}~g(\bigwedge b)$，即$v~\mathcal{S}~(\bigwedge b)$.
\end{proof}
