...
\begin{enumerate}
	\item $g(\top_B) = \top_A$;
	\item $g(x \wedge y) = f(x) \wedge f(y)$.
\end{enumerate}
...
\begin{definition}
\rm The correctness relation $\mathcal{S} \subseteq V \times M$ for the new analysis is defined as follows
$$
...
{\color{red} 但是在实际中我们不会直接通过上面诱导出来的analysis transfer functions，因为构造起来太过于繁琐，常常会用一个$h'$来安全的替换它的使用}

\begin{definition}
\rm {\color{red} (新的传递函数)} For the new analysis transfer function, we can choose any function $\func{h'}{D'}{D'}$ such that
$$
h \geq f \circ h_n \circ g.
$$
\end{definition}
