...
\begin{proof}
\rm 由Lemma \ref{lfp-construct}，这里显然$g(\alpha) = f^\alpha(\perp)$，但是$\mathbb{N} < \xi$，因此这里并不能直接把lfp找出来，从而我们只能写成一个不等式. 如果加强$f$使之变成一个meet homomorphism，那么我们可以直接证明$\bigvee\Set{f^\alpha(\perp)}{\alpha \in \mathbb{N}}$是一个fixed point，即 
$$
...
\begin{proof}
这是因为总有$f(f^n_\nabla) \geq f^n_\nabla$，所以$f^m_\Delta \in \text{Ext}(f)$, 自然地有$f^m_\nabla \leq \bigvee \text{Ext}(f) = \text{gfp}(f)$.
\end{proof}
...
\begin{proof}
这个证明过程是trivial的，我们需要证明$f(f^{n-1}_\Delta) \leq f^{n-1}_\Delta$. 当$n=1$时，有$f(f^m_\nabla) \leq f^m_\nabla$，假设$n=k, k \geq 1$有$f(f^{k-1}_\nabla) \leq f^{k-1}_\nabla$成立. 当$n=k+1$时，有
$$
f(f^k_\nabla)= f(f^{k-1}_\nabla \nabla f(f^{k-1}_\nabla)) \leq f(f^{k-1}_\nabla) \leq f^{k-1}_\nabla .
$$
\end{proof}
