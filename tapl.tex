...
%why
\begin{annotation}
\rm 这里很重要的理解: 如果给定一个type relation $\termtype{\term{t}}{\text{T}}$，那么肯定是由上述inference rule推导出来的，所以我们会经常看到从conclude推premise的过程，也就是寻找合适的inference rule反向推导，这个过程我们称其为\bluet{derivation}，其中反向寻找合适的inference rule的方法是利用了所谓inversion lemma. 
\end{annotation}
...
\begin{case}
$\term{t} = \ifelse{\term{t}_1}{\term{t}_2}{\term{t}_3} ~~~~\text{subterm}:~ \term{t}_1 = \text{bool} ~~ \term{t}_2 = \text{T} ~~ \term{t}_3 = \text{T}.$

由归纳假设subterm are not stuck term, 当$\term{t}_1 = \true$或者$\term{t}_1 = \false$时，我们对$\term{t}$一步evaluation得到$\term{t}_2$或者$\term{t}_3$.  另外当$\term{t}_1 \to \term{t}_1'$时，我们也可以得到$\term{t} \to \ifelse{\term{t}_1'}{\term{t}_2}{\term{t}_3}$. 
\end{case} 

\begin{case}
$\term{t} = \succt{\term{t}_1} ~~~ \text{subterm}:~ \term{t}_1 = \text{nat}.$
...
当$\term{t}_1 = \true$或者$\term{t}_1 = \false$时，我们对$\term{t}$一步evaluation得到$t' = \term{t}_2$或者$t' = \term{t}_3$，显然$t'$ with $T$.  另外当我们take $\term{t}_1 \to \term{t}_1'$时，此时对subterm $t_1$可以apply induction hypothesis得到$\term{t}_1':bool$，因此$\term{t}' = \ifelse{\term{t}_1'}{\term{t}_2}{\term{t}_3}$，此时依然有$\term{t}'$ with $T$.  
\end{case}
...
\begin{annotation}
\rm (\redt{evaluation rules和typing rules的关系}) Progress和preservation两个theorem将evaluation和typing联系了起来. 想象如果对没有typing支持的finite string根据evaluation rule不断地做reduction，我们最终会得到怎样的一个结果呢？ 肯定是一个normal form, 除了前面提到的values: variables和lambda abstractions的，也许我们还可以得到$x y$其中$x$ is variable. 但有了typing之后我们就可以排除这样的东西，同时是否可以考虑将finite string推广至infinite呢？
\end{annotation}
...
\begin{definition}
\rm Let $S,T$ be any types, $S$ is a \redt{subtype} of $T$ if any term of type $S$ can safely be used in a context where a term of type $T$ is expected, simply written $\subtyp{S}{T}$. 
\end{definition}
...
$$
\begin{array}{ll}
\subtyp{\{l_1:T_1,l_2:T_2,\cdots,l_{n+k}:T_{n+k}\}}{\{l_1:T_1,l_2:T_2,\cdots,l_n:T_n\}} & \text{width subtyping} \\[0.5em]
...
\begin{proof}
证明过程是根据typing rules归纳well-typed $t:T$，这里只需要重点关心new typing rules，和稍微有些区别的canonical forms和substitution. i.e. representative cases. 
\begin{case}
$$
...
\begin{case}
$$
\infer{\Gamma \vdash t_1~t_2:T_2}{\Gamma \vdash t_1:T_1 \to T_2 & \Gamma \vdash t_2:T_1}
$$
evalutaion现在只考虑app-abs，设$t_1 = \lam{x:S_{11}}{t_{12}}$，注意这里用的argument annotation是$S_{11}$，因为这里没有东西可以说明它是$T_1$，然后我们根据Lemma \ref{app-subsum}，才能得到关于$S_{11}$的信息$\subtyp{T_1}{S_{11}}$和$\Gamma,x:S_{11} \vdash t_{12}:T_2$. 于是再对$t_2:T_1$用一下subsumption rule得到$t_2:S_{11}$，最后用Lemma  \ref{subtyp-subs}即可. 
\end{case}
\end{proof}
...
\begin{proof}
与在STLC证明类似，此时注意下canonical forms即可. 
\end{proof}
...
\begin{proof}
\rm 假设存在$\vdash v:Bot$，你可以用subsumption推出$v$可以是任意的types，例如你如果考虑$v:Top \to Top$和$v:\{\}$，你再用一下它们对应的canonical forms，但是显然一个term不可能有两种形态.

%可以用canonical forms两个情况来说明:若$v$是一个abs $\lam{x:T_1}{t_2}$, 那么对于任意地type $S_1 \to S_2$我们应当有如下的derivation:
%$$
%\infer{\vdash \lam{x:T_1}{t_2}:S_1 \to S_2}{ \infer{\lam{x:T_1}{t_2}:T_1 \to T_2}{x:T_1 \vdash t_2:T_2} & \deduce{\subtyp{T_1\to T_2}{S_1 \to S_2}}{\subtyp{S_1}{T_1} &\subtyp{T_2}{S_2}}}
%$$
%似乎对所以的func types找到一个common bottom type不太好找，这里显然是需要$\lam{x:T_1}{t_2}:Top \to Bot$. 那么换个思路，我们希望找到是某个type的subtyp只有它自己，那它实际就是bottom type，that is amazing.
\end{proof}

\begin{annotation}
\rm 对于top type而言我们非常自然地将它移除或者添加，它并不会对某个type system的properties造成影响，但是bottom type似乎没有那么容易做到，例如当你添加bottom type之前，如果$t_1~t_2$ is well-typed application, 那么我们肯定知道$t_1$是一个arrow type，当你添加完bottom type之后，这里就变成了$t_1$可能是一个arrow type或者bottom type，会将我们typechecker复杂化. 但是它也有两个非常好的性质:
\begin{enumerate}
	\item 因为它对应的empty terms，我们可以让它作为某些没有result value的操作，例如某些函数没有return value. 
	\item 因为它的safety in any contexts，它又可以表示任意的值，这时我们可以让它作为在trigger exceptions的返回值. 
\end{enumerate}
\end{annotation}
