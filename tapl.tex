...
\begin{enumerate}
	...
	\item 再做根据type derivations的induction，但是这个过程存在一个难点存在于abstraction $ \lam{\termtype{x}{\singletype{T}_1}}{\term{t}_2}$上，其中$\term{t}_2$不一定是closed的，因为$\term{x}$可能是在$\term{t}_2$作为free variable出现的，而你此时无法说明$\text{SN}_{\singletype{T}_2}(\term{t}_2)$. 
\end{enumerate}
\end{annotation}
