...
\begin{annotation}
\rm 前面提到的neural term-"neutral terms contain a free variable at a 'head' position"，它的意思就是指形如$\term{x}~\term{y}$，其中$\term{x}$是一个free variable，而$\term{y}$是一个lambda abstraction. 
\end{annotation}
...
$$
这里cyclic reduction的过程,$l_1$的type依赖$l_2$的type依赖，反过来$l_2$的type依赖$l_1$的type. 那么如何构造这样一个$\mathcal{L} \to T$的map呢？ 它是可以在evaluation过程动态构造的，因为只要一个location第一次被allocated，那么在它对应的位置上一定有一个具体的type，同样无论后面经历assignment多少次都只有唯一的type对应，这样我们可以一开始就将$\Sigma$置为一个empty map，再根据对应的操作是维护它就可以了. 
\end{annotation}
