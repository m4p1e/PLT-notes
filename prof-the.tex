...
\rm 证明的Hthe的sketch:
\begin{enumerate}
	\item 首先把它放到cut-free sequent calculus里面，根据cfsc的completeness得到一个proof:
	\[
	...
	\]
	\item 下一步就是从上往下把existential quantifier，即去掉$R \exists$ rule，直接把primise换成conclusion，相当于就是一个$\vn{id}$ rule. 这里你会发现有些关于existential quantifier的$\vn{contraction}$也是用不了，因此也要去掉. 最终我们就得到了Hthe的结论. 
\end{enumerate} 
