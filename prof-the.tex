...
首先不考虑这么复杂的问题，我们显式区分开来某个formula has verfication or is just hypothesis for use，先从结构(shape)上来分析问题. 不过上面的例子也暗示了我们可以找到一个automation来帮助我们进行推导，如果如今存在的local reduction会让证明过程过程变成不确定，也就是说我们可能得到很多不同的proofs，解决这个问题也是我们引入verification和uses目的\cite{15-317-vu}. 
\end{annotation}
