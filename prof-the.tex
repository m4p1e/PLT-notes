...
\begin{enumerate}
	...
	father(hu, maple).
	\end{tabular}\newline\newline
	Each of those is an atom and they are interpreted as a logical formula: e.g., the first fact is $mother(li, maple) \supset \top$, \bluet{but why?}
...
\end{enumerate}
当我们有了上面这些clauses之后，我们就可以来做一些querys, 例如parent(X, maggie)表示谁是maggie的parent，prolog以它作为current goal，开始搜索head是parent的那些caluses，找到对应的body里面的subgoals，接着周而复始，直到最后facts，这过程充斥着我们提到的backtracking和unification. 
\end{example}
