...
\begin{annotation}
\rm natural deduction实际上一种constructive logic或者intuitive logic,  与classic logic有所区别, 例如proposition $A \vee (A \supset B)$ is true在classic logic中是accepted，因为我们可以根据真值表来验证. 但是在natural deduction里面我们需要一些explicit evidences，例如你得首先说明$A~true$或者$(A \supset B)~true$才行，这里似乎就会被卡住.  
\begin{verbatim}
1    assume -(A + (A => B)).
2      assume A.
3        A + (A => B).                                     +I1 2
...
7        F.                                                =>E 5,6
8        B.                                                Efq 7
9      therefore A => B.                                   =>I 6,8
10     A + (A => B).                                       +I2 9
11     F.                                                  =>E 1,10
12   therefore --(A + (A => B)).                           =>I 1,11
13   A + (A => B).                                         Raa 12    	    
\end{verbatim}
这里引入了negation introduction, falsehood introduction and double negation使得natural deduction变成complete才能解决这个问题. 

首先不考虑这么复杂的问题，我们显式区分开来某个formula has verfication or is just hypothesis for use，先从结构来分析问题. 
\end{annotation}
...
\begin{theorem}
\rm Given hypotheses $\Gamma = (A_1 \uparrow, \cdots, A_n \uparrow)$, it correpsonds to $\widehat{\Gamma} = (A_1~left,\cdots,A_n~left)$. Then we have
\begin{enumerate}
\item If $\Gamma \vdash C \uparrow$ then $\widehat{\Gamma} \vdash C~right$;
	\item If $\Gamma \vdash A \downarrow$ and $\widehat{\Gamma} \vdash A~left \vdash C~right$ then $\widehat{\Gamma} \vdash C~right$. 
\end{enumerate}
\end{theorem}
...
$$
\begin{aligned}
&(1) &&\widehat{\Gamma}, A_2~left \vdash C~right && \text{assumaption} \\
