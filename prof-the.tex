...
\begin{annotation}
\rm 在前一章里面我们把ND转化成了ND in sequent nation，这一章我们就来把verification calculus转化为sequent calculus. 
\end{annotation}


\begin{annotation}
\rm 为了更好的说明verification calculus和sequent calculus的对应，我们下面在定义sequent calculus的时候，会象征性地加上$left$和$right$对应use和verification.
\end{annotation}
...
\begin{annotation}
\rm 像我们之前见到exchange (把judgement的某一端里面的成员交位置), contraction, weakening这些都属于structural rules. 这里我们即将提到的linear logic就和它们有一点关系. linear logic关注的对象是resources，或者更准确的说是resource availability，现在假设你拥有两个相同的resources或者拥有一个resources，在语境上不一样的，这里nlab上用了一个形象的例子，假如我们有这样的sequent
\[
...
\]
如果你用一下contraction，语境就不正确了:
\[
	\text{have cake} \vdash \text{have cake}~\wedge ~\text{eat cake}
\]
我们不能在仅拥有一块蛋糕的情况下，选择吃掉了它之后还剩下一块蛋糕，这显然是不正确的. 因此我们将从sequent calculus开始，小心地去掉contraction和weakening来建立linear logic. 首先我们得扩充一下我们之前的sequent，我们之前的sequent全都是\emph{additive form}，这里来扩充一下在classic logic和intuitionistic logic下等价的另外一种形式\emph{multiplicative logic}，相比于additive form它具有\redt{更弱一些premises}，下面给出它们的对比:
\begin{center}
...
\hline\text{Conjunction}& $\with$ & $\otimes$ \\
\hline\text{Disjunction}& $\oplus$ & $\parr$
\end{tabular}
\end{center}
\end{annotation}
