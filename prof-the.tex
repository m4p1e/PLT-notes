...
\begin{proof}
\rm ($\Leftarrow$) 如果$(W, \rho_a, \rho_b)$is Kripke frame where $\rho_a$ is the reflexive, transitive closure of $\rho_b$. 对于一个formula只要注意到$\square_a\square_a p \to \square_a\square_b p$即可，可以从需要考虑的successors数量来证明. 对于第二个formula，我有点不太确定，因为$p \to \square_b p$和$p \to \square_a p$在$K$里面都是vaild.
\end{proof}
