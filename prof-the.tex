...
\begin{annotation}
\rm 在前一章里面我们把ND转化成了ND in sequent nation，这一章我们就来把verification calculus转化为sequent calculus. 为了更好的说明verification calculus和sequent calculus的对应，我们下面在定义sequent calculus的时候，会象征性地加上$left$和$right$对应use和verification.
\end{annotation}
...
\begin{annotation}
\rm 像我们之前见到几种操作: exchange (把judgement的某一端里面的成员交位置), contraction, weakening这些都属于structural rules. 后面我们即将提到的linear logic就和它们就有着密切的关系，linear logic关注的对象是resources，或者更准确的说是resource availability. 现在假设拥有两个相同的resources和拥有一个resources，在语境上是不一样的，这里nlab上用了一个形象的例子，假如我们有这样的sequent
\[
...
我们知道classic and intuitionistic logic的deduction都可以依赖sequent calculus, 那么我们linear logic是否也可以依赖sequent calculus呢？ 尽管我们现在还不知道linear logic大体上怎样的，但是我们知道the quantity of resources is the property of linear logics，因此一个明显地问题是其中contraction和weakening如何处理? 首先我们得扩充一下我们之前的sequent，我们之前的sequent全都是\emph{additive form}，这里来扩充一下在classic logic和intuitionistic logic下等价的另外一种形式\emph{multiplicative logic}，相比于additive form它具有\redt{更弱一些premises}，下面给出它们的对比:
\begin{center}
...
\end{tabular}
\end{center}
删掉$\vee R$的原因是它有right side不唯一，就目前而言的single-conclusion intuitionistic logic中是不被允许的. 为了使说明更加的清晰，我们现在用不同符号来表示相同的connective在不同的形式中，即
\begin{center}
\begin{tabular}{c|c|c}
