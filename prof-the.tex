...
\usepackage{array}% for adjust tabular column widths
...
上面这段话出自natural deduction的wiki，这个$u$指明了assumption $A~true$的作用域是从$A~true$到implications introducation inference line，直接说明了$A\supset B$并不依赖assumption $A~true$, 只是$B~true$依赖$A~true$. 这就是所谓的discharge，$A~true$在作用域以外是invisible的. 实际上这里的$\supset I$就是deduction theorem
$$
...
\begin{example}
\rm Local reduction for $\wedge I$ introduction rule.
\[
	\infer[\wedge E_1]{A~true}{\infer[\wedge I]{A \wedge B}{ \deduce{A~true}{\mathcal{D}} & \deduce{B~true}{\mathcal{E}}}} \Rightarrow_R \deduce{A~true}{\mathcal{D}}
\]
Similarily for $\wedge E_2$.
\end{example}

\begin{example}
\rm Local expansion for $\wedge E$ elimination rule.
\[
	\deduce{A \wedge B~true}{\mathcal{D}} \Rightarrow_E \infer[\wedge I]{A \wedge B~true}{\infer[\wedge E_1]{A~true}{\deduce{A \wedge B~true}{\mathcal{D}}} & \infer[\wedge E_2]{B~true}{\deduce{A \wedge B~true}{\mathcal{D}}}}
\]
\end{example}
...
$$
We also can give the elimination rule of negation.
$$
\infer[\neg E]{\perp}{\neg A & A}
$$
so $\perp$ actually means any contradictions, moreover double negation is coming. NOTICE! if your add $\neg E$ to natural deduction, then we obtain the deduction system for classic propositional logic. its literally excluded middle. 
\end{example}

\begin{annotation}
\rm notaional definition可以看做用已有的东西构造出一些东西. 与之对应的是我们可以直接符号化的给出某个新的定义，称之为\redt{symbolic definition}. 
\end{annotation}
...
还是前面那个true proposition, 对应type不变, 用$\eta$ conversion把前面的term换一下
$$
\lam{z}{\lam{x}{\left<\lam{y}{\pi_1(x~y)},\lam{y}{\pi_2(x~y)} \right>}}z
$$
那这是不是违反Curry-Howard isomorphsim了呢？ 其实并不是，这里的对应是指proof terms和program terms, 显然program typing过程变了，对应的proof deduction也要变. 所以这里衍生出一个有趣的问题, $\eta$ conversation和$\beta$ conversation对应到proof deduction过程上是什么呢?  
\end{example}
...
\begin{annotation}\label{pre-H-system}
\rm 在Gentzen提出ND之前，存在一种\redt{Hilbert-style systems}, 或者叫\redt{axiomatic system}，它不同于ND完全是由general inference rules组成的，它只存在一种inherent rule:
\[
\infer{\vdash B}{\vdash A \supset B & \vdash A}
\]
其他的rules需要你自己给定，称其为axioms.
\end{annotation}


\begin{annotation}
\rm The "\redt{BHK-conditions}" (for Brouwer-Heyting-Kolmogorov), which give the explanations of logical operations of propositional logic in terms of direct provability of propositions, can be put as follows:
\begin{itemize}
	\item A direct proof of the proposition $A \wedge B$ consists of proofs of the propositions $A$ and $B$.
	\item A direct proof of the proposition $A \vee B$ consists of a proof of the proposition $A$ or a proof of the proposition $B$.
	\item A direct proof of the proposition $A \supset B$ consists of a proof of the proposition $B$ from the assumption that there is a proof of the proposition $A$.
	\item A direct proof of the proposition $\perp$ is impossible.
\end{itemize}
这样的思考方式使得ND完完全全展现了我们CS里面所需要的\redt{computability}.
\end{annotation}

\begin{definition}
\rm A deduction is in \redt{normal form} if there is no formula which is a conclusion of an introduction rule and the main premise of the elimination rule of the same connective.
\end{definition}

\begin{example}
\rm 例如下面deduction就是不是normal form:
\[
	\infer[\wedge I]{A \wedge B}{\infer[\wedge E_1]{A}{A \wedge B} & \infer[\wedge E_2]{A}{A \wedge B}}
\]
\end{example}
...
\begin{annotation}
\rm (\redt{detail of correspondence}) 其中$\Gamma$是一个set of formulas，它可以是empty set. 思考上述sequent形式下的natural deduction，我们应该用bottom-up的视角来观察. 试想我们在没有additional assumptions证明一个formulas，在最开始$\Gamma$应该是empty的，随着我们不断apply上述规则过程中将不断的填充$\Gamma$. 那么什么时候证明算接结束了呢？ 在natural deduction中我们从下往上使用introduction rules，并添加相应的assumptions，再从上往下使用emilination rules，直到它们在中途相遇，这时候我们的证明就结束了，而且此时所有额外的assumptions都应该被discharge了，sequent calculus中并没有显式的discharge的操作，我们是通过把某个conclusion依赖的assumption都列出来代替的，在利用sequent构造proof的时候，总是以hyp rule结束的, 这个hyp rule实际上是一个特殊地discharge:
\[
	\infer[\supset I^u]{A \supset A}{A^u}
\]
...
\begin{definition}\label{vc-form-sq}
\rm A sequent is a pariticular form of hypothetical judgement
$$
...
\begin{annotation}
\rm (\redt{关于duplication of main formula的解释}) 这 里frank给出的rules都有一个特点: conclusion里面的assumptions依然出现在了primises里面，这是因为"monotonicity of hypotheses"的概念: 在bottom-up形式下的proof中一旦建立某个assumption，那么它在后续的derivation中同样available. 那么你可能回想最后还留着这么多assumption，还怎么用init rule来close掉证明呢？ 这里不用担心，我们是可以用一些structural rules来去掉这些冗余的assumption的. 
\end{annotation}

\begin{annotation}
\rm \redt{关于vertification calculus和sequent calculus对应关系} 它们最基本formula的关系在Definition \ref{{vc-form-sq}} 里面已经提到了. 这里还有一个关于inference rules关系: introduction rules in vertification calculus对应right rules in sequent calculus; elimination rules in vertification calculus对应left rules in sequent calculus.
\end{annotation}

\begin{lemma}
\rm (\redt{Subformula property}) All formulas in a sequent calculus derviration are subformulas of the endsequent of the derivation. 
\end{lemma}

\begin{annotation}
\rm (\redt{初见proof-searching}) 现在你在sequent calculus里面做derivation，可以完全使用linear bottom-up手法来做，而不用像在natural deduction里面你要从分不同的方向使用introduction rules和elimination rules. 这是因为比如你现在手上现阶段要做derivation的formula为$\Gamma \Rightarrow A$, 一旦你确定了其中的principle formula是哪个，我们再直接选择一个inference rule，就可以唯一的确定primises. 
\end{annotation}
...
\begin{definition}
\rm A \redt{derivation} in sequent calculus is either an \emph{init}, an instanse of $\perp L$, or an application of a logical rule to derivations concluding its premises. 
\end{definition}

\begin{definition}
\rm The \redt{height of a derivation} is the greatest number of successive application of rules in it, where \emph{init} and $\perp L$ have height $0$.
\end{definition}


\begin{annotation}
\rm 在height of derivation做induction是一个非常常见并且有用的方法，在后面我们会经常看见. 我可以使用下面这样的一个notation来让我们induction过程更加的清晰明了:
\[
	\vdash_n \Gamma \Rightarrow C
\]
它表示"the sequent $\Gamma \Rightarrow C$ in G3ip is derivable with a height of derivation at most $n$".
\end{annotation}
...
\begin{annotation}\label{permute-cut}
\rm rank deduction = ? "premutes it up" 就是指将某个cut rule"往上移"， 例如rank deduction
$$
\begin{gathered}
...
\end{gathered}
$$
此时\emph{cut rule}中的两个premises中的cut formula(main formula of \emph{cut} rule)同时也是apply在两个premise上rules的main formula，这种shape的cut我称为\redt{principal cases}.
\end{annotation}

\begin{definition}
\rm We call \redt{grade reduction} the rewriting operation that transforms a cut on a formula into a cut on simple formulas(i.e, formulas with fewer connectives) or simpler proofs.
\end{definition}

\begin{annotation}
\rm 在annotation \ref{permute-cut}中的第二个例子中两个\emph{cut}分别对应了grade reduction中的两种情况: 其中第一个\emph{cut}将原来的cut formula变得simpler了; 第二个\emph{cut}将原来的left branch proof变得更加simpler了. 注意这两个\emph{cut}都在和最初的那个比较. 
\end{annotation}
...
\begin{proof}
这个lemma简而言之就是不能在principal cases做rank deduction, 其他情况都能做. 你得考虑\emph{cut} rule的premises可以被applied的所有rules. 试着做一个case的证明$\Gamma = \Gamma', B_1 \wedge B_2$:
\[
	\begin{gathered}
	\infer[\vn{cut}]{\Gamma', B_1 \wedge B_2 \Rightarrow C}{\infer[\wedge L_1]{\Gamma', B_1 \wedge B_2 \Rightarrow A}{\deduce{\Gamma', B_1 \wedge B_2, B_1 \Rightarrow A}{\mathcal{D}}} & \deduce{\Gamma', B_1 \wedge B_2,A \Rightarrow C}{\mathcal{E}}} \\
	\rotatebox{270}{$\leadsto$}\\
	\infer[\wedge L_1]{\Gamma', B_1 \wedge B_2 \Rightarrow C}{\infer{\Gamma', B_1 \wedge B_2, B_1 \Rightarrow C}{\deduce{\Gamma', B_1 \wedge B_2, B_1 \Rightarrow A}{\mathcal{D}} & \deduce{\Gamma', B_1 \wedge B_2, B_1, A \Rightarrow C}{\mathcal{E} + ~\vn{weakening}}}}
	\end{gathered}
\]
之前的$\wedge L_1$在\emph{cut}上面，这就是lemma里面想表明的，但是这还不够我们得把所有这样的rules都表达出来，所以还得做induction, 两个primises合起来就20个cases.. TD 
\end{proof}
...
\begin{annotation}
\rm 注意\emph{cut} rule是在用the vertification of $A$去替换the use of $A$，这和前面substitution of uses是不太一样的. 显然引入\emph{cut} rule将会破坏我们之前提到的sublformula property.
\end{annotation}
...
\begin{proof}
\rm 证明\emph{cut} rule is admissible是个技术活. 我们要做一个nested structure induction. 首先给定\emph{cut} rule的shape
$$
...
\begin{annotation}
\rm \bluet{引入\emph{cut} rule不利于做proof search}, 因为可能引入的$A$有很多种可能，其中部分可能并不是原来formula的subformula.   
\end{annotation}
...
\hline $\infer[\vee L]{\Gamma, A \vee B \to C}{\Gamma, A \to C & \Gamma, B \to C}$ & $\begin{aligned}\infer[\vee R_1]{\Gamma \to A \vee B}{\Gamma \to A} \\ \infer[\vee R_2]{\Gamma\to A \vee B}{\Gamma \to B} \end{aligned}$ \rule{0em}{3em}\\
...
%\[
%\infer[weakning]{\Gamma, A \supset B, B \vdash C}{\Gamma, B \vdash C} \quad\quad \infer[cut]{\Gamma, B \vdash C}{\infer[id]{\Gamma, A \supset B \vdash A \supset B}{} & \Gamma,A \supset B, B \vdash C}
%\]
\begin{annotation}
...
这个结论实际很有趣，我们并不能在intuitionistic logic证明law of execluded middle，但是它的double negation却可以证明.
\end{annotation}
...
\rm that is
$$
\infer[cut]{\Gamma, A \to B}{\Gamma, A \to A \supset B & \infer{\Gamma,A, A\supset B \to B}{\infer[init]{\Gamma, A, A \supset B \to A}{} & \infer[init]{\Gamma,A, B \to B}{}}}
$$
\end{proof}

\begin{annotation}
\rm \redt{为什么the left rule of implication is not invertible?} 因为从conclusion推不出left premise，但是right premise是可以推出来的. 
\end{annotation}

\begin{annotation}
\rm 如果不\emph{cut} rule, 我们可以尝试我们前面提到的一种在height of derivation上做induction的方法来证明invertibility.  
\end{annotation}

\begin{lemma}
\rm If $\vdash_n \Gamma, A \wedge B \to C$, then $\vdash_n \Gamma, A, B \to C$.
\end{lemma}

\begin{proof}
\rm Base case: 当$n = 0$时
\[
	\infer[id]{\Gamma', C, A \wedge B \to C}{} \quad\quad \infer[\perp L]{\Gamma', \perp, A \wedge B \to C}{}
\]
其中$\Gamma = \Gamma', C$，这里我们用了\emph{id} rule (admissible)来\emph{init} rule这是一个小技巧，让我们不用再费力对$C$做induction. 显然上面两种情况都是可以得到$\vdash_0 \Gamma, A, B \Rightarrow C$. 我们假设当height $n$情况下成立. 

Induction cases: 当$\vdash_{n+1} \Gamma, A \wedge B \to C$，这里要分两种情况:(1) 如果$A \wedge B$ is main formula for the last sequent, 那么
\[
	\infer{\Gamma, A \wedge B \to C}{\deduce{\Gamma, A, B \to C}{\mathcal{D}}}
\]
显然$\mathcal{D}$为$\vdash_n \Gamma, A, B \Rightarrow C$; (2)如果$A \wedge B$ is not main formula for the end sequent, 那么我们最多有whatever rules, there are most two primises for the last sequent, 设其为$\Gamma_1, A \wedge B \to C_1$和$\Gamma_2, A \wedge B \to C_2$, 这里我们再用一下induction hypothesis可以得到$\vdash_n \Gamma_1, A, B \to C_1$和$\vdash_n \Gamma_2, A, B \to C_2$. 最后我们再用一下这个derivation里面的topmost rule即可得到所需要的结论.  
\end{proof}
...
\begin{definition}
\rm \redt{G3cp} is invertiable sequent calculus for classical logic:
\[
	\begin{gathered}
		\infer[init]{\Gamma, P \to P, \Delta}{} \\
		\infer[\wedge L]{\Gamma, A \wedge B \to \Delta}{\Gamma, A, B \to \Delta} \quad \quad \infer[\wedge R]{\Gamma \to A \wedge B ,\Delta}{\Gamma \to A, \Delta & \Gamma \to B, \Delta}\\
		\infer[\vee L]{\Gamma, A \vee B \to \Delta}{\Gamma, A \to \Delta, \Gamma, B \to \Delta} \quad \quad \infer[\vee R]{\Gamma \to A \vee B, \Delta}{\Gamma \to A ,B, \Delta}\\
		\infer[\supset L]{\Gamma, A \supset B \to \Delta}{\Gamma \to A, \Delta & \Gamma, B \to \Delta} \quad\quad  	\infer[\supset R]{\Gamma \to A \supset B, \Delta}{\Gamma, A \to B, \Delta} \\
		\infer[\perp L]{\Gamma, \perp \to \Delta}{} \quad\quad \infer[\top R]{\Gamma \to \top}{}	
	\end{gathered}
\] 
\end{definition}

\begin{theorem}
\rm All rules of G3cp are invertible.
\end{theorem}

\begin{annotation}
\rm 这章存在理由是因为有一个非常有趣的观点，通过proof-search我们可以来构造一些关于propositional formula的normal forms (i.e CNF, DNF). 对于任意一个formual $C$, 我们都可以对sequent $\to C$做derivation实际得到的是一个树, 其中$\to C$就是这个树的root. 当$\to C$ is vaild, 那么其所有的叶子结点都应该以\emph{init}或者$\perp L$. 如果$\to c$ is not vaild, 那么一定存在叶子结点是无法继续被decomposed. 首先我们来刻画一下所有可能的叶子结点处的sequents应该具有的形式. 
\end{annotation}

\begin{definition}
\rm The form of topsequents of a proof-search:
\[
	P_1,\cdots,P_m,\perp,\cdots,\perp \to Q_1,\cdots,Q_n, \perp, \cdots,\perp
\]
where $P_i$ and $Q_j$ are atoms and the number of $\perp$ in the antecedent or succedent as well as $m$ or $n$ can be $0$.
\end{definition}

\begin{lemma}
\rm The decomposition of a sequent $\Gamma \to \Delta$ into topsequents in G3cp is \redt{unique}.
\end{lemma}

\begin{definition}
\rm A \redt{regular sequent} is a sequent of the form $P_1, \cdots, P_m \to Q_1, \cdots, Q_n, \perp, \cdots, \perp$ where for all $i,j$, $P_i \neq Q_j$, the antecedent is empty if $m = 0$, the succedent is $\perp$ if $n = 0$. The trace formula of a regular sequent is
\begin{itemize}
	\item if $m > 0, n > 0$, then $P_1 \wedge \cdots \wedge P_m \supset Q_1 \vee \cdots \vee Q_n$.
	\item if $m = 0, n > 0$, then $Q_1 \vee \cdots \vee Q_n$.
	\item if $m > 0, n = 0$, then $\neg (P_1 \wedge \cdots \wedge P_m)$.
	\item if $m=0, n=0$, then $\perp$.
\end{itemize}
where possible repetitions of the $P_i$ or $Q_j$ in the regular sequent are deleted. 
\end{definition}

\begin{annotation}
\rm 上面展示的就是一种特殊的sequent到propositional form的对应关系，实际上就是在proof search中无法被close掉的topsequent.
\end{annotation}

\begin{theorem}
\rm (\redt{CNF}) A propositioal formula $C$ is equivalent to the conjunction of the trace formulas of its decomposition inio regular sequents.
\end{theorem}

\begin{proof}
\rm 假设decomposition(derivation) of $\to C$中topsquents为$\Gamma_1 \to \Delta_1, \cdots, \Gamma_m \to \Delta_m$，其中前$n$ ($m \geq n$)个为regular sequents，它们对应的trace formual为$C_1, \cdots, C_n$. 我们需要同时证明$C \supset C_1\wedge \cdots \wedge C_n$和$C_1\wedge \cdots \wedge C_n \supset C$，即证明$\to C \supset C_1 \wedge \cdots \wedge C_n$ and $\to C_1\wedge \cdots \wedge C_n \supset C$ are both valid.

\bluet{$\to C \supset C_1\wedge \cdots \wedge C_n$} 我们给当前这个derivation里面所有的sequents的left side都加上$C$. 那么我们可以从$\Gamma_1, C \to \Delta_1, \cdots, \Gamma_m, C \to \Delta_m$沿着原来derivation的paths推出end sequent $C \to C$. 由于$C \to C$ is valid, 因此$\Gamma_1, C \to \Delta_1, \cdots, \Gamma_m, C \to \Delta_m$肯定也都是valid. 那么就是说$C \to C_i$ is valid，那么只需要重复$\wedge R$就能得到$C \to C_1 \wedge \cdots \wedge C_n$.

\bluet{$\to C_1\wedge \cdots \wedge C_n \supset C$} 同理我们给当前这个derivation里面所有的sequent的left side都加上$C_1 \wedge \cdots \wedge C_n$，最后得到end sequent为$C_1\wedge \cdots \wedge C_n \to C$. 对于topsequents中的regular sequents我们都有$\Gamma_i, C_1\wedge \cdots \wedge C_n \to C_i$, 显然现在它们都是valid了, 因此$C_1\wedge \cdots \wedge C_n \to C$.

这样的证明方式正确性来源于G3cp里面rules都是same contexts.
\end{proof}

\newpage
\subsection{Structral Rules}
...
\begin{definition}
\rm The \redt{weakening rule} is defined as
\[
	\infer[weakening]{\Gamma, A \to C}{\Gamma \to C}
\]
\end{definition}

\begin{definition}
\rm The \redt{contraction rule} is defined as
\[
	\infer[contract]{\Gamma, A, \to C}{\Gamma, A, A \to C}
\]
\end{definition}

\begin{annotation}
\rm 在前面我们已经在频繁使用weakening和contraction rules了，这是完全是因为它们在之前定义的sequent calculus里面是admissible的(\bluet{缺一个严格的proof! TODO}). 这里我们给出为什么它们是admissible的原因，或者说为什么不需要单独定义:
\begin{itemize}
	\item weakening: 因为我们的initial rule是
	\[
		\infer[init]{\Gamma, P \to P}{}
	\]
	而不是
	\[
		\infer[axiom]{P \to P}{}
	\]
	\item contraction: 因为我们定义的inference rules是shared contexts或者same contexts, 例如$\vee L$
	\[
		\infer[\vee L]{\Gamma, A \vee B \to C}{\Gamma, A \to C & \Gamma, B \to C}
	\]
	而不是
	\[
	\infer[\vee L]{\Gamma,\Delta, A \vee B \to C}{\Gamma, A \to C & \Delta, B \to C}
	\]
\end{itemize}
这些variations在sequent calculus中非常常见，你要非常小心注意它们的出现是否会改变一些properties.
\end{annotation}


\newpage
\subsection{Terminating Sequent calculus}

%\begin{remark}
%\rm If a system is both \redt{cut-free and contraction-free}, it can have the property that the primises are proper parts of the conclusion. In this case, we have bottom-up proof search resulting in a tree that \redt{terminates}!
%\end{remark}
...
\begin{definition}
\rm We call simplified sequent calculus with above compound left rules except original left implication \redt{G4ip}, discovered by Hudelmaier and Dyckhoff(1992).
\end{definition}
...
\begin{annotation}
\rm (\redt{sequent-calculus扩展过程中的局限性}) Cut-elimination是一个sequent calculus里面一个非常强的技术，因为它可以很好保持subformula property. 但是在实际中如果我们想扩展sequent calculus，往里面加点mathematical axioms，这个时候可能derivations就无法继续保持cut-free了，例如加两个axioms:
\[
	\infer[axiom_1]{\to A \supset B}{} ~~~~~ \text{and} ~~~~~  \infer[axiom_2]{\to A}{}
\]
那么关于sequent $\to B$的derivation如下:
\[
	\infer[cut]{\to B}{\infer[axiom_1]{\to A}{} & \infer[cup]{A \to B}{\infer[axiom_2]{\to A \supset B}{} & \infer[\supset L]{A \supset B, A \to B }{\infer{init}{A\supset B, A \to A}{} &  \infer[init]{B \to B}{}}}}
\]
此时是没有办法去掉cut rule, 因为你根本找不到可以apply到$\to B$上的rules. 如果我们想继续保持cut-free怎么办呢？ 有一种办法就是只引入nonlogical inference rules，形如:
\[
	\infer{P \to C}{Q \to C}
\]
其中$P,Q$都是atomic formula. 
\end{annotation}
...
\[
	\infer[\vn{resolution}]{(C_i \cup C_j)\sigma}{C_i \cup \{p\} & C_j \cup \{\overline{p'}\}}
\]
...
\begin{definition}
\rm Simple sequent calculus for propostional resolution:
$$
\begin{aligned}
\infer{\Gamma \vdash C}{C \in \Gamma} \quad\quad \infer{\Gamma \vdash C_i \cup C_j}{\Gamma \vdash C_i \cup \{p\} & \Gamma \vdash C_j \cup \{\overline{p}\}}
\end{aligned}
$$
A refutation of $\Gamma$ is a derivation ending in $\Gamma \vdash \emptyset$ where $\emptyset$ is empty clause.
\end{definition}
...
%像我们之前见到几种操作: exchange (把judgement的某一端里面的成员交位置), contraction, weakening这些都属于structural rules. 后面我们即将提到的linear logic就和它们就有着密切的关系，
\begin{annotation}
\rm (\redt{初见linear logic}) linear logic关注的对象是resources，或者更准确的说是resource availability. 现在假设拥有两个相同的resources和只拥有一个resource，在语境上是不一样的，这里nlab上用了一个形象的例子，假如我们有这样的sequent
\[
...
\]
可以看见两者在对premises处理上是不一样的，前者需要“消耗”premises(注意它和discharge是不一样的，discharge本质上一种identity)，而后者可以依然保持premises. 你可以预见structural rules将会在linear logic building过程中作为障碍出现. 
\end{annotation}
...
\begin{tikzpicture}[scale=0.2]
\tikzstyle{every node}+=[inner sep=0pt]
\draw [black] (23.9,-27.3) circle (3);
\draw (23.9,-27.3) node {$Locked$};
\draw [black] (23.9,-27.3) circle (2.4);
\draw [black] (43.1,-27.3) circle (3);
\draw (43.1,-27.3) node {$Unlocked$};
\draw [black] (41.606,-29.89) arc (-38.29609:-141.70391:10.329);
\fill [black] (41.61,-29.89) -- (40.72,-30.21) -- (41.5,-30.83);
\draw (33.5,-34.32) node [below] {$Coin$};
\draw [black] (24.647,-24.407) arc (156.61151:23.38849:9.646);
\fill [black] (24.65,-24.41) -- (25.42,-23.87) -- (24.51,-23.47);
\draw (33.5,-18.09) node [above] {$Push$};
\draw [black] (45.78,-25.977) arc (144:-144:2.25);
\draw (50.35,-27.3) node [right] {$Coin$};
\fill [black] (45.78,-28.62) -- (46.13,-29.5) -- (46.72,-28.69);
\draw [black] (21.22,-28.623) arc (324:36:2.25);
\draw (16.65,-27.3) node [left] {$Push$};
\fill [black] (21.22,-25.98) -- (20.87,-25.1) -- (20.28,-25.91);
\end{tikzpicture}
\end{center}
根据上图我们可以得到下面的axioms:
\begin{itemize}
	\item $T_1 = locked \otimes coin \multimap unlocked$.
	\item $T_2 = locked \otimes push \multimap locked$.
	\item $T_3 = unlocked \otimes coin \multimap unlocked \otimes coin$ (注意这里我们保留了coin).
	\item $T_4 = unlocked \otimes push \multimap locked$
\end{itemize}
那么我们可以来试着证明一下$coin \multimap unlocked$:
\[
	\infer[\multimap R]{locked, \oc T_1, \oc T_2, \oc T_3, \oc T_4 \to coin \multimap unlocked}{\infer[\oc L]{locked, \oc T_1, \oc T_2, \oc T_3, \oc T_4, coin \to unlocked}{\infer[\multimap L]{locked, T_1, \oc T_2, \oc T_3, \oc T_4, coin \to unlocked}{\infer[\otimes R]{locked, coin \to locked \otimes coin}{\infer[init]{locked \to locked}{} & \infer[init]{coin \to coin}{}} & \infer[\oc wL^* + init]{\oc T_2, \oc T_3, \oc T_4, unlocked \to unlocked}{}}}}
\]
\end{example}

%\newpage
%\subsection{Focused Linear logic}
...
\newpage
\section{Structural Proof analysis of Axiomatic Theories}

\begin{remark}
In this chapter, we give a method of adding axioms to sequent calculus, in the form of nonlogical rules of inference.
\end{remark}

\subsection{From Aximos to Inference Rules}

\begin{definition}
\rm In \redt{nonlogical rules}, the premises and conclusion are sequents that have atoms as main formulas in the antecedent and an arbitrary context in the succedent. The most general scheme with shared contexts, is 
\[
	\infer[\vn{reg}]{P_1, \cdots, P_m, \Gamma \to \Delta}{Q_1, \Gamma \to \Delta & \cdots & Q_n, \Gamma \to \Delta}
\]
where $\Gamma, \Delta$ are arbitrary multisets, $P_1,\cdots,P_m,Q_1,\cdots,Q_n$ are fixed atoms, and the number of premises $n$ can be zero. 
\end{definition}

%\begin{definition}
%\rm The axioms always can be represented by $P_1 \wedge \cdots \wedge P_m \supset Q_1 \vee \cdots Q_n$.
%\end{definition}

%\begin{annotation}
%\rm 其中$P_1 \wedge \cdots \wedge P_m \supset Q_1 \vee \cdots Q_n$就是Section \ref{resolution}里面提到过的形式.
%\end{annotation}

%\begin{annotation}
%\rm 之前在\ref{pre-H-system}里面简单地提到了Hilbert-style system, 你有没有想%过如何把它转换到sequent calculus里面来呢？ 我们得为此构造一个特殊sequent %calculus，让它里面只包含一个之前我们提到rule
%\[
%	\infer{\to B}{\to A \supset B & \to A}
%\]
%对于每一个axiom $A$我们可以将其转换为$\infer{\to A}{}$. 类似地我们如果想要证明某个formula $C$, 只需要对$\to C$做derivation即可. 
%\end{annotation}

\newpage
\section{Proof of Program}

\subsection{Hoare Logic}

\begin{definition}
\rm The \redt{Hoare triple} is of the form 
\[
	\{P\}C\{Q\}
\]
where $P$ and $Q$ are assertions(predicates) and $C$ is a command. $P$ is named the precondition and $Q$ is named the postcondition, that is when we start executing $C$ in a state satisfying $P$ the any final state will satisfy $Q$.
\end{definition}

\todo{Partial and total correctness}

\todo{Some basic aximos: skip, assignment, composition...}

\subsection{Weakest Preconditions and Strongest Postconditions}

\begin{definition}
\rm Given a command $C$ with postcondition $Q$, the \redt{weakest precondition} $P$ for $\{C\}Q$ is a predicate such that for any precondition $P'$ satisfying $P'\{C\}Q$ if and only if $P' \supset P$, written as $\vn{wp}(C,Q) = P$.
\end{definition}

\begin{definition}
\rm Given a command $C$ with precondition $P$, the \redt{strongest postcondition} $Q$ for $P\{C\}$ is a predicate such that for any precondition $Q'$ satifies $P\{C\}Q'$ if and only if $Q \supset Q'$, written as $\vn{sp}(C,P) = Q$.
\end{definition}

\begin{annotation}
\rm 如何理解weakest和strongest? 单纯从logic上理解: 若$P \supset Q$, 那么$P$ is stronger than $Q$, 或者说$Q$ is weaker than $P$. 再来回答第二个问题: 为什么只有weakest precondition而没有strongest precondition? \cite{15-414-post} 想想falsehood $\perp$, 按照定义它可以是任意$\{C\}Q$的precondition, 你可以找到一个比它还强的precondition吗? 显然只能是它自己, 因此这样strongest precondition实际上trivial的. 同理, truth $\top$可以是任意的$\{P\}C$的postcondition, 比它还弱的也只能是它自己.   
\end{annotation}


\todo{weakest precondition calculus}
\newpage
...
\bibitem{proof-theory-development} Chapter 12, Resolution.
\newline\url{https://plato.stanford.edu/entries/proof-theory-development/#SeqCalLatDev}
\bibitem{15-414-post} Lecture Notes on Weakest Preconditions and Strongest Postconditions.
\newline\url{https://www.cs.cmu.edu/~15414/lectures/11-post.pdf}
\end{thebibliography}
\end{document}
