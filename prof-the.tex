...
\begin{itemize}
	...
	\begin{enumerate}
		...
		\item If $G$ contains a clause $\{p\}$. we can give a interpretation makes $p$ is false then $G$ is false, but $G$ needs to be false for every interpretation, thats is not a problem. If given $p$ is true under some interpretations, in the case, we can remove the clause $\{p\}$, every clause that contains $p$(a disjunction with a true literal is true),  and every clause that contains $\overline{p}$($\overline{p}$ is the negation of $p$, that is a false literal in a disjunction is good for nothing).  
